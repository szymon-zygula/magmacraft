...
impl VulkanStateBuilder {
    ...
    pub fn build(mut self) -> Result<VulkanState, VulkanError> {
    ...
    fn get_ready_for_state_creation(&mut self) -> Result<(), VulkanError> {
        self.init_entry()?;
    ...
    fn init_entry(&mut self) -> Result<(), VulkanError> {
        self.entry.set(Rc::new(ash::Entry::new()?));
        Ok(())
    }

    fn init_instance(&mut self) -> Result<(), VulkanError> {
        let mut instance_builder = vulkan::instance::Instance::builder()
                .entry(Rc::clone(self.entry.get()))
                .version(0, 0, 0)
                .name("Magmacraft")
    ...
    fn init_extension_loaders(&mut self) {
        let instance_handle = self.instance.get().get_handle();
    ...
    fn add_instance_debug_extension(&mut self) -> Result<(), VulkanError> {
        if self.debug_mode {
            let extension_name = ash::extensions::ext::DebugUtils::name()
                .to_str().unwrap();

            self.instance_extensions.get_mut()?
                .push(extension_name);
        }
    ...
    fn init_debug_messenger(&mut self) -> Result<(), VulkanError> {
        if self.debug_mode {
            self.debug_messenger.set(
                Some(vulkan::debug_utils::DebugMessenger::new(
                    Rc::clone(&self.debug_utils_loader.get()),
                    Rc::clone(&self.instance.get())
                )?)
            );
        }
        else {
            self.debug_messenger.set(None);
        }
